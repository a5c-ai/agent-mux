...
[~] - TUI gap fill (2026-04-13): added `skills-view` (hotkey `k`), `agents-view` (hotkey `g`), `hooks-view` (hotkey `h`). All three support interactive delete (j/k navigate, d with y/n confirm, r refresh). Help-view documents every view hotkey. 6 tests. Still outstanding: add/install flow inside TUI.
