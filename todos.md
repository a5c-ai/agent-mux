...
[~] - TUI gap fill (2026-04-13): added `skills-view` (hotkey `k`), `agents-view` (hotkey `g`), `hooks-view` (hotkey `h`). Agents-view now supports interactive delete (j/k navigate, d to delete with y/n confirm, r to refresh). Help-view documents every view hotkey. 4 tests total. Still outstanding: extend interactive delete to skills-view + hooks-view; add add/install flow.
