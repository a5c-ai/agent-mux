...
[~] - TUI gap fill (2026-04-13): added `skills-view` (hotkey `k`) and `agents-view` (hotkey `g`) so the new CLI skill/agent commands have matching read-only browsers in the TUI. 2 tests. Still outstanding: hooks-view, remote-view, interactive add/remove actions.
