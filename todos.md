...
[~] - TUI gap fill (2026-04-13): added `skills-view` (hotkey `k`), `agents-view` (hotkey `g`), `hooks-view` (hotkey `h`) — read-only browsers matching the CLI commands. 3 tests. Still outstanding: interactive add/remove actions (remote is stateful install, not a list view).
