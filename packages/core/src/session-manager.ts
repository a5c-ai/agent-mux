...
export class SessionManagerImpl implements SessionManager {
  ...
  private static readonly FAST_LIST_BUFFER = 10;
  ...
  async list(agent: AgentName, options?: SessionListOptions): Promise<SessionSummary[]> {
    ...
    if (this._canFastListByDate(options, sortField)) {
      return this._listByRecentFiles(adapter, filePaths, limit, sortDir);
    }
    ...
    for (const fp of filePaths) {
      try {
        ...
        const { createdAt, updatedAt } = this._getSessionDates(session);

        // Apply date filters
        if (options?.since && createdAt < options.since) continue;
        if (options?.until && createdAt > options.until) continue;

        summaries.push(this._buildSummary(session, createdAt, updatedAt));
      } catch {
  ...
  private _canFastListByDate(options: SessionListOptions | undefined, sortField: string): boolean {
    return (
      sortField === 'date' &&
      options?.since == null &&
      options?.until == null &&
  ...
  private async _listByRecentFiles(
    adapter: ReturnType<SessionManagerImpl['_getAdapter']>,
    filePaths: string[],
    limit: number,
    sortDir: 'asc' | 'desc',
    ...
    const summaries: SessionSummary[] = [];
    const parseBudget = Math.max(limit + SessionManagerImpl.FAST_LIST_BUFFER, limit * 2);
  ...
  private _getSessionDates(session: {
    createdAt: string;
    updatedAt: string;
