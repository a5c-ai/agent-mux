...
// OpenAI SDK types (would normally be imported from 'openai')
...
export class CodexSdkAdapter extends BaseProgrammaticAdapter {
      ...
      for await (const chunk of stream) {
        if (chunk.choices && chunk.choices[0]) {
          ...
          if (choice.finish_reason === 'function_call' && currentFunctionCall && toolCallId) {
            ...
            const functionResult = await this.executeMockFunction(
  ...
  /**
   * Create chat completion stream (mock implementation).
  ...
  /**
   * Execute mock function calls.
   */
  private async executeMockFunction(name: string, arguments_: string): Promise<string> {
    try {
  ...
  /**
   * Extract cost information from OpenAI usage object.
   */
