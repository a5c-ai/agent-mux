          ...
          {activeSessions.map((session) => (
            <article key={session.sessionId} className="list-card">
              ...
              <div className="actions">
                <Link className="ghost-link" to={`/sessions/${session.agent}/${session.sessionId}`}>
                  Open chat
                </Link>
                {session.activeRunId ? (
                  <Link className="ghost-link" to={`/runs/${session.activeRunId}`}>
                    Attach live view
                  </Link>
                ) : null}
              </div>
          ...
          {inactiveSessions.map((session) => (
            <article key={session.sessionId} className="list-card">
              ...
              <div className="actions">
                ...
                <Link className="ghost-link" to={`/sessions/${session.agent}/${session.sessionId}?compose=1`}>
                  Continue
                </Link>
                {session.latestRunId ? (
                  <Link className="ghost-link" to={`/runs/${session.latestRunId}`}>
                    Last live run
                  </Link>
                ) : null}
              </div>
